...
echo "[run_libero_finetune] $(date) starting WAN run (pi05_libero_wan_precomp)"
HF_HOME=$HF_HOME XLA_PYTHON_CLIENT_MEM_FRACTION=0.95 uv run scripts/train.py \
    pi05_libero_wan_precomp \
    --exp-name=libero_wan_full_finetune \
    --overwrite
echo "[run_libero_finetune] $(date) WAN run exited with code $?"

# ---- Run 2: plain pi05 LoRA baseline (no VEGA) ----
echo "[run_libero_finetune] $(date) starting baseline run (pi05_libero)"
HF_HOME=$HF_HOME XLA_PYTHON_CLIENT_MEM_FRACTION=0.95 uv run scripts/train.py \
    pi05_libero \
    --exp-name=libero_full_finetune_NOWAN \
    --overwrite
